...
class sentistrength:
    def __init__(self, config=dict()):
        self.negasi = [line.replace('\n','') for line in open("./lib/sentistrength/negatingword.txt").read().splitlines()]
        self.tanya = [line.replace('\n','') for line in open("./lib/sentistrength/questionword.txt").read().splitlines()]
        #create sentiment words dictionary
        self.sentiwords_txt = [line.replace('\n','').split(":") for line in open("./lib/sentistrength/sentiwords_id.txt").read().splitlines()]
    ...
    def classify(self):
        ...
        try:
            ...
            print ("error: {0} {1} ".format(self.sentences_max_pos, self.sentences_max_neg))
        return result
