...
'''
reviewslib.py
This library contains functions needed for preprocessing and data cleaning.
...
TABLE KEDUA, reviews, "id" MEMILIKI KONEKSI DENGAN "id" DI TABEL PERTAMA.
BUAT KOLOM BARU DI DATAFRAME, PAKE dataframe.insert(loc, column='test', value=0)
'''
# modules needed
import pymongo
...
def dftosqlite(dataframe, tablename, collection, conn):
    ...
    for i in range(len(dataframe['id'])):
        ...
        reviewdf.to_sql(tablename, con=conn,
...
def productdf(dfid, collection):
    ...
    # Change columns name
    df.columns = ['id', 'answerscnt', 'cashback', 'rone', 'rtwo', 'rthree', 'rfour',
            'rfive', 'discussionscnt', 'helpfulreviewerscnt','merchantname', 'reputation', 
            'merchanttype', 'category', 'proddiscussions', 'prodname', 'prodpic', 'price',
            'prodrating', 'prodseen', 'prodsold', 'subcat', 'subsubcat',
            'questionscnt', 'reviewcount', 'topads', 'uri']
    # Reorder the columns
    ...
    df['cashback'] = df['cashback'].str.replace('%', '').replace('no cashback', '0').replace(',', '.')
