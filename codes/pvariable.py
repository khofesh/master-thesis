...
def createCashVal(connection, cursor):
    '''
    Create cashbackval column in prodpage table
    '''
...
def createPosNegRev(connection, cursor):
    '''
    create posreview and negreview columns in prodpage table.
    '''
    sqlc = 'ALTER TABLE prodpage ADD COLUMN posreview REAL'
    cursor.execute(sqlc)
    connection.commit()

    sqlc1 = 'ALTER TABLE prodpage ADD COLUMN negreview REAL'
...
def createOtherAgreeMean(connection, cursor):
    '''
    Create otheragreemean column in prodpage table
    '''
    sqlc = 'ALTER TABLE prodpage ADD COLUMN otheragreemean'
    cursor.execute(sqlc)
    connection.commit()
...
def createRatingMostHelpful(connection, cursor):
    '''
    Create ratingmosthelpful column in prodpage table
    '''
...
def createRevRep(connection, cursor):
    '''
    Create reviewersRep column in prodpage table
    '''
...
def createRevPicTotal(connection, cursor):
    '''
    create revpictotal column in prodpage table.
    '''
...
def createProdPicsTot(connection, cursor):
    '''
    create a column in prodpage table.
    name of a column: prodpicstotal
    '''
    sqlc = '''
        ALTER TABLE prodpage ADD COLUMN prodpicstotal INTEGER
    '''
    cursor.execute(sqlc)
    connection.commit()
...
def createMinbeli(connection, cursor):
    sqlcommand = 'ALTER TABLE prodpage ADD COLUMN minbeli INTEGER'
...
def createActRevCnt(connection, cursor):
    '''
    create column actualrevcount in prodpage table
    '''
...
def prodpageTrain(connection, cursor, csvtrain):
    '''
    ...
    # save to csv file
    traindata.to_csv(csvtrain, index=False)

def createSalesClus(connection, cursor):
    '''
    Create cashbackval column in prodpage table
    '''
...
# sale rank
csvoutput = './csvfiles/output_pvariable.csv'
#salesRank(conn, c, csvoutput)

# minimum purchase quantity
#createMinbeli(conn, c)
csvinput = './csvfiles/output_minbeli.csv'
#minPurchase(conn, c, csvinput, csvoutput)

# picture of products
#createProdPicsTot(conn, c)
#prodPics(conn, c, csvoutput)

# reviewers reputation
#createRevRep(conn, c)
#reviewersRep(conn, c, csvoutput)

# reviewers profile picture
#createRevPicTotal(conn, c)
#defavadir = 'defaultavatar'
#reviewerPic(conn, c, csvoutput, defavadir)

# ratingMostHelpful
#createRatingMostHelpful(conn, c)
#ratingMostHelpful(conn, c, csvoutput)

# number of people who find reviews helpful
#createOtherAgreeMean(conn, c)
#otherAgree(conn, c, csvoutput)

# posReview and negReview
#createPosNegRev(conn, c)
#posNegReview(conn, c, csvoutput)

# cashback value ~ discount value
#createCashVal(conn, c)
#cashbackVal(conn, c, csvoutput)

# actual reviewers count
#createActRevCnt(conn, c)
#actualRevCount(conn, c, csvoutput)

# Sales cluster
#createSalesClus(conn, c)
#csvcluster = './csvfiles/salesclusters.csv'
#salesCluster(conn, c, csvcluster)

# ranking (average method)
#meanRank(conn, c, csvoutput)

# update prodsold, whose value <= actualrevcount
#updateProdsold(conn, c, csvoutput)
# run meanRank() after updating prodsold
...
# Create 'sentimenttraining' table
#trainSentiment(conn, c)

# Update 'sentimenttraining' table
#csvsentiment = './csvfiles/output_sentiment.csv'
#upTrainSentiment(conn, c, csvsentiment, 'sentimenttraining')

# create csv for model training
#csvtrain = './csvfiles/output_training.csv'
#prodpageTrain(conn, c, csvtrain)
