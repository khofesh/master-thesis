...
class ProductPageSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # Get the "urlproduct" database
        self.db = self.client.urlproduct
        # Get the "url" collection
        self.collection = self.db.url
        # Get everything 
        # (contains "url", "topads", "_id")
        self.documents = self.collection.find()
        for i in self.documents:
            yield scrapy.Request(i['url'], callback=self.parse)
    ...
    def parse(self, response):
        ...
        if urlCount == 5:
            sleep(10)
            self.logger.info(
                    'sleeping for 10 seconds before processing the next URL'
                    )
